package com.quickly.xqw.module.news.article;
...
public class NewsArticlePresenter implements NewsContrat.Presenter{


    private static final String TAG = "NewsArticlePresenter";

    private NewsContrat.View view;

    private List dataList = new ArrayList<>();

    private Random random = new Random();

    public NewsArticlePresenter(NewsContrat.View view) {
        this.view = view;
        // 混乱排版 加入额外的布局数据
        addLayout();
    }
    ...
    private Observable<NewsArticleBean>  getRandom(){
        ...
        Log.i(TAG, "getRandom: " + cursor);
        return RetrofitFactory.getRetrofit(HttpUri.SERVER_IP).create(INewsApi.class).getRecommend(cursor);
