...
public class RetrofitFactory8080 {
    ...
    public static Retrofit getRetrofit() {
        if (retrofit == null) {
            synchronized (RetrofitFactory8080.class) {
                if (retrofit == null) {
                    // 指定缓存路径,缓存大小 50Mb
                    Cache cache = new Cache(new File(XQWApplication.appContext.getCacheDir(), "HttpCache"),
                            1024 * 1024 * 50);

                    // Cookie 持久化
                    ClearableCookieJar cookieJar = new PersistentCookieJar(new SetCookieCache(), new SharedPrefsCookiePersistor(XQWApplication.appContext));
                    ...
                    retrofit = new Retrofit.Builder()
                            .baseUrl(HttpUri.SERVER_IP_8080)
